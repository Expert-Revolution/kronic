...
def health_check():
    ...
    try:
        ...
        return jsonify({"status": "unhealthy", "version": "v1", "error": str(e)}), 503
...
@api_v1.route(
    "/namespaces/<namespace>/cronjobs/<cronjob_name>/suspend", methods=["POST"]
)
@namespace_filter
@auth_required
def suspend_cronjob(namespace, cronjob_name):
    ...
    try:
        ...
        log.error(
            f"Failed to toggle suspend for cronjob {cronjob_name} in namespace {namespace}: {e}"
        )
        return jsonify({"error": "Failed to toggle cronjob suspend status"}), 500


@api_v1.route(
    "/namespaces/<namespace>/cronjobs/<cronjob_name>/trigger", methods=["POST"]
)
@namespace_filter
@auth_required
def trigger_cronjob(namespace, cronjob_name):
    ...
    try:
        ...
        log.error(
            f"Failed to trigger cronjob {cronjob_name} in namespace {namespace}: {e}"
        )
