...
class K3dClusterManager:
    ...
    def create_cluster(self):
        ...
        try:
            ...
            container_kubeconfig_path = self.kubeconfig_path.replace(
                ".yaml", "-container.yaml"
            )
            try:
        ...
        if self.kubeconfig_path:
            ...
            if os.path.exists(container_kubeconfig_path):
                try:
                    ...
                    print(
                        f"✓ Container kubeconfig file removed: {container_kubeconfig_path}"
                    )
                except Exception:
                    pass
...
def start_cluster(cluster_name="kronic-localdev"):
    ...
    if manager.cluster_exists():
        ...
        try:
            ...
            container_kubeconfig_path = kubeconfig_path.replace(
                ".yaml", "-container.yaml"
            )
            try:
...
def main():
    parser = argparse.ArgumentParser(
        description="Manage k3d clusters for Kronic local development"
    )
    parser.add_argument(
