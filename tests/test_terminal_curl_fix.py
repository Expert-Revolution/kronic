...
class TestTerminalCurlFix(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.container = {
            'command': 'echo "hello world"'  # Initial command
        }
        ...
        try:
            ...
            if '-d' in self.container['command']:
                ...
                if re.search(d_param_regex, self.container['command']):
                    self.container['command'] = re.sub(d_param_regex, replacement, self.container['command'])
            else:
    ...
    def simulate_format_json_for_curl_new(self):
        ...
        try:
            ...
            terminal_curl_command = f'curl -v -X POST "{endpoint}" -d \'{compact_json}\''
    ...
    def test_new_behavior_replaces_command(self):
        ...
        self.assertIn('-d \'{"accessToken":"string","username":"RPA","userId":"0"}\'', self.container['command'])
    ...
    def test_new_behavior_extracts_endpoint_from_existing_command(self):
        ...
        self.container['command'] = '["curl", "-v", "-X", "POST", "https://api.myservice.com/webhook", "-d", "{\\"key\\": \\"value\\"}"]'
        self.simulate_format_json_for_curl_new()
