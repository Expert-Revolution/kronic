...
def test_react_login_ui():
    ...
    with app.test_client() as client:
        ...
        ), f"JS bundle not accessible: {js_response.status_code}"
        assert (
            css_response.status_code == 200
        ), f"CSS bundle not accessible: {css_response.status_code}"
        ...
        login_response = client.post(
            "/api/auth/login", json={"email": "test@example.com", "password": "invalid"}
        )
        ...
        assert register_response.status_code in [
            ...
            405,
            501,
        ], f"Unexpected register response: {register_response.status_code}"
        ...
        missing_js = [
            ...
            if indicator.lower() not in js_content.lower()
        ]
        missing_css = [
            indicator for indicator in css_indicators if indicator not in css_content
        ]

        # Allow some missing indicators as the build might optimize them out
        assert (
            len(missing_js) <= 2
        ), f"Too many missing React indicators in JS: {missing_js}"
