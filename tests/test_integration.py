...
class KindClusterManager:
    ...
    def create_cluster(self):
        ...
        try:
            # Create kind cluster
            cmd = ["kind", "create", "cluster", "--name", self.cluster_name, "--wait", "60s"]
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=120)
            if result.returncode != 0:
                raise RuntimeError(f"Failed to create kind cluster: {result.stderr}")
            
            # Export kubeconfig
            cmd = ["kind", "export", "kubeconfig", "--name", self.cluster_name, "--kubeconfig", self.kubeconfig_path]
    ...
    def _wait_for_cluster_ready(self, timeout=60):
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                pass
            time.sleep(2)
        raise RuntimeError("Cluster failed to become ready within timeout")
...
class TestKronicIntegration:
    ...
    def test_cluster_connectivity(self, kind_cluster):
        ...
        assert any(node.status.conditions[-1].type == "Ready" and 
    ...
    def test_get_cronjobs_integration(self, kind_cluster, sample_cronjob_yaml):
        ...
        try:
            ...
            assert cronjob_details is not False, "get_cronjob should return the cronjob details"
    ...
    def test_cronjob_suspend_toggle_integration(self, kind_cluster, sample_cronjob_yaml):
        ...
        try:
            ...
            assert "error" not in result, f"Toggle operation should not return error: {result}"
            
            # Verify cronjob is now suspended (toggled from False to True)
            cronjob_details = get_cronjob("default", "test-cronjob")
            assert cronjob_details["spec"]["suspend"] is True, "Cronjob should now be suspended"
            ...
            cronjob_details = get_cronjob("default", "test-cronjob")
            assert cronjob_details["spec"].get("suspend", False) is False, "Cronjob should no longer be suspended"
    ...
    def test_trigger_cronjob_integration(self, kind_cluster, sample_cronjob_yaml):
        ...
        try:
            ...
            assert len(jobs) > 0, "At least one job should have been created by triggering the cronjob"
            
            # Verify the job has the correct owner reference
            job = jobs[0]
            assert job["metadata"]["name"].startswith("test-cronjob-"), "Job name should be based on cronjob name"
    ...
    def test_namespace_filtering_integration(self, kind_cluster, sample_cronjob_yaml):
        ...
        try:
            ...
            test_cronjob_found = any(cj["name"] == "test-cronjob" for cj in cronjobs_in_test_ns)
            ...
            test_cronjob_in_default = any(cj["name"] == "test-cronjob" for cj in cronjobs_in_default)
            assert not test_cronjob_in_default, "Test cronjob should not be found in default namespace"
            
            # Test getting cronjobs from all namespaces should include our test cronjob
            all_cronjobs = get_cronjobs()
            test_cronjob_in_all = any(cj["name"] == "test-cronjob" and cj["namespace"] == "test-namespace" 
                                    for cj in all_cronjobs)
            assert test_cronjob_in_all, "Test cronjob should be found when listing all namespaces"
            
        finally:
            # Cleanup
            subprocess.run(
                ["kubectl", "delete", "cronjob", "test-cronjob", "-n", "test-namespace", "--ignore-not-found"],
                capture_output=True, timeout=30
            )
            subprocess.run(
