...
class TestPostgresScheduling(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.chart_path = os.path.join(os.path.dirname(__file__), '..', 'chart', 'kronic')
    ...
    def _get_postgres_statefulset(self, documents):
        ...
        for doc in documents:
            if (doc and 
                doc.get('kind') == 'StatefulSet' and 
                doc.get('metadata', {}).get('name', '').endswith('-postgres')):
                return doc
        return None
    ...
    def test_default_scheduling_empty(self):
        ...
        self.assertNotIn('affinity', spec, "affinity should not be present with empty default values")
        self.assertNotIn('tolerations', spec, "tolerations should not be present with empty default values") 
        self.assertNotIn('nodeSelector', spec, "nodeSelector should not be present with empty default values")
