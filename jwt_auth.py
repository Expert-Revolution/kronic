...
try:
    ...
    log.warning(f"Redis connection failed: {e}. Rate limiting and sessions will use memory.")
    redis_client = None
    ...
    def verify_token(token: str, token_type: str = 'access') -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if payload.get('type') != token_type:
                log.warning(f"Invalid token type. Expected {token_type}, got {payload.get('type')}")
                return None
...
class SessionManager:
    ...
    @staticmethod
    def store_session(user_id: str, session_data: Dict[str, Any], ttl: int = 3600) -> bool:
